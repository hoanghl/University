		...
		for (i = 0; i < pl_size-1; i++) {
			...
			for (j=i+1;j<pl_size;j++)
			{
				if (pl[i].arrivaltime>pl[j].arrivaltime) {
					swap (&pl[i].arrivaltime,&pl[j].arrivaltime);
					swap (&pl[i].bursttime,&pl[j].bursttime);
					swap (&pl[i].number,&pl[j].number);
				}
			}
		}
	}
...
void* sched_fcfs() //1
{
   ...
   while (l <pl_size)
   {
	 ...
	 int i;
	 for (i=0;i<pl[l].bursttime;i++)
	 {
		pthread_mutex_lock(&mutex);
		printf(" Time %2d-%2d Proccess %5d\n",timecounter,timecounter+1,pl[l].number);
		timecounter++;
		pthread_mutex_unlock(&mutex);
		delay(750);
	 }
	 printf(" Time %5d Proccess %5d end\n",timecounter,pl[l].number );    
