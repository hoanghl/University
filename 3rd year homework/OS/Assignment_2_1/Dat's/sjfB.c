...
void* sched_fcfs() //1
{
   ...
   while (l <pl_size)
   {
	...
	if (k<pl_size)
	for (index=l;index<pl_size;index++)
		if (timecounter>=pl[index].arrivaltime) k++;
	index=0;
	temp=1;
	for (index=0,temp=1;temp<k;temp++)
	{
		if (pl[index].bursttime>pl[temp].bursttime & pl[temp].check==0) index=temp;
		else if (pl[index].check==1) index=temp;
	}
	if (pl[index].check==0 & k>0)
	{
		int n;
		pl[index].assigned_timeslot[0].starttime = timecounter;
		for (n=0;n<pl[index].bursttime;n++)
		{
			pthread_mutex_lock(&mutex);
			printf(" Time %2d-%2d Proccess %5d \n",timecounter,timecounter+1,pl[index].number);
			timecounter++;
			pthread_mutex_unlock(&mutex);
			delay(500);
		}
		printf(" Time %5d Proccess %5d end\n",timecounter,pl[index].number );
		pl[index].check=1;
		pl[index].assigned_timeslot[0].endtime = timecounter;
         	pl[index].timeslot_count = 1;
		l++;
	}
	else
	{
