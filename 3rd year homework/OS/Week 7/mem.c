...
void * best_fit_allocator(unsigned int size) {
	 ...
	 * Basically, the algorithms of two are similar in the second part: split the hole
	 * The first part: while first-fist stops at the firstly found hole, best-fit traverses every posible ones and choose which has the smallest size
	 */
	...
	/* Tommy */
	// find appropriate hole for allocating
	do {
		if (size <= current_region->size) {
			...
			/* Tommy */
			// find min hole			
			if (bestfit_region == NULL)						// first time
				bestfit_region = current_region;
			else{
				if (bestfit_region->size > current_region->size)
					bestfit_region = current_region;
			}
		}
		current_region = current_region->next;
